...
@section('content')
    <x-backend.page-title heading="Products" can="edit role" url=""/>
    <div class="mx-4 mt-5 h-full">
        <x-splade-form action="{{ route('products.update', $product) }}"
                       method="PUT"
                       :default="$product">
            ...
            <x-splade-data remember="menu" default="{ general_tab: true, seo_tab: false}">
                ...
                <aside v-show="data.images_tab">
                    <div class="flex flex-col sm:flex-row mt-4">
                        <x-splade-file name="image" filepond preview class="w-full sm:w-1/2 mr-8 mt-4 sm:mt-0" label="Main Image" />
